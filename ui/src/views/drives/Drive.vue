    ...
    <v-flex xs12>
      ...
      <v-card>
        <v-toolbar dense flat>
          ...
          <v-btn small icon :to="{name:'Drives'}">
        ...
        <v-card-text class="pa-0">
           ...
           <FileExplorer type="drive" :driveOrShareId="$route.params.driveId" @fileSizeChanged="onFileSizeChange"></FileExplorer>
...
export default {
  ...
  methods: {
    async getDriveInfo() {
      this.driveInfo = await Drives.getMy(this.driveId)
    },
    onFileSizeChange(){
      clearTimeout(tid)
      tid=setTimeout(()=>{
